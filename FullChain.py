...
def start_tmux(session):
    ...
    os.system(f"""gnome-terminal --tab -e "bash -c 'tmux new -s BOX';bash" & """)
...
def start_nmap(syntax,domain,session):
    ...
    os.system(f"tmux select-pane -t 1 && tmux send 'nmap -oA nmap_scan.txt {syntax} {domain}' ENTER")
...
def start_nikto(domain,port,session):
    ...
    if port == 443:
        ...
        domian = 'http://' + domain
    os.system(f"tmux select-pane -t 2 && tmux send 'nikto -url {domain} -port {port} -o nikto_scan.txt ' ENTER")
...
def start_gobuster(domain,port,session):
    ...
    if port == 443:
        domain = 'https://' + domain
        os.system(f"tmux select-pane -t 3 && tmux send 'gobuster dir -u {domain} -k -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -t 10' ENTER")
    elif port == 80:
        domain = 'http://' + domain
        os.system(f"tmux select-pane -t 3 && tmux send 'gobuster dir -u {domain} -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -t 10' ENTER")
    else:
