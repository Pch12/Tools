...
#domain validation
if [ ! $1 =~ "(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]" ];then
    echo "Enter a proper domain, try again.. "
    exit 1
fi
...
nmap -iL final.txt -T5 -oA ./scans/scanned.txt

echo "=======Running EyeWitness======="
#This depends on where your EyeWitness is installed. change the directory path
/root/Documents/tools/EyeWitness/EyeWitness.py -f $pwd/probed.txt -d $1 -t 5
mv /root/Documents/tools/EyeWitness/EyeWitness.py/$1 $pwd/eyewitness/
